...
public abstract class BaseSandboxFilesystem implements AbstractSandboxFilesystem {
    ...
    public EditResult edit(
        ...
        // The python script is passed as a double-quoted shell argument to `python3 -c`.
        // Each statement MUST end with a REAL newline ("\n"), NOT a literal backslash-n
        // ("\\n"): bash does not interpret \n inside double quotes, so "\\n" would collapse
        // the whole script onto one line and raise a Python SyntaxError. The heredoc below
        // already uses real newlines for the same reason.
        String cmd =
